...
frame_num = 15; % number of frames to operate
...
exercise4(paddedFileName, paddedWidth, paddedHeight, frame_num, i, r, QP, intra_para);
...
function exercise4(yuv_path, width, height, num_frames, i, r, QP, intra_para)
    ...
    prev_mvs = zeros(height/i, width/i, 2);
    ...
    for frame = 1:num_frames
        ...
        curr_mvs = zeros(height/i, width/i, 2);


        %if intra_para == 0
        if intra_para == 100
            intra_reconstructed_frame = zeros(size(Y));
            for row = 1:i:height
                for col = 1:i:width
                    block = Y(row:min(row+i-1, height), col:min(col+i-1, width)); % current block
                    %[dx, dy] = block_search(reference, block, row, col, r);
                    intra_reference = intra_reconstructed_frame; 
                    [predicted_block,mode] = intra_search(intra_reference, block, row, col, i);
                    ...
                    intra_reconstructed_frame(row:min(row+i-1, height), col:min(col+i-1, width)) = reconstructed_block;
        ...
        %intra prediction
        %elseif mod(frame,intra_para+1) == 1
         elseif intra_para == 99
            intra_reconstructed_frame = zeros(size(Y));
            for row = 1:i:height
                for col = 1:i:width
                    block = Y(row:min(row+i-1, height), col:min(col+i-1, width)); % current block
            ...
            for row = 1:i:height
                for col = 1:i:width
                    ...
                    %r_idx
                    %c_idx
                    curr_mvs(r_idx, c_idx, :) = [dx, dy];
                    
                    d_dx = dx - prev_mvs(r_idx, c_idx, 1);
                    d_dy = dy - prev_mvs(r_idx, c_idx, 2);               
                    % Save motion vector
                    fprintf(mv_fid, '%d,%d,%d,%d\n', row, col, d_dx, d_dy);
