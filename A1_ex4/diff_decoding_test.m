...
function decoder(mv_path,  width, height, i, num_frames, QP)
    ...
    for frame = 1:num_frames
        ...
        % Read the approximated residual block data from the binary file
        %coeff_bin = 'encoded_file_frame_test.txt';
        ...
        for row = 1:i:height
            for col = 1:i:width
                ...
                r_idx = (row+7)/i;
                c_idx = (col+7)/i;
                % Calculate actual motion vector
                dx = d_dx + prev_mvs(r_idx, c_idx, 1);
                dy = d_dy + prev_mvs(r_idx, c_idx, 2);
...
function binaryString = readBinary(filename)
    ...
    %frame_length = fread(fileId, 1, 'uint32');  % read frame length
    ...
    %binaryString = fread(fileId, frame_length);
    % Close the file
    fclose(fileId);
end
